...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Dark\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521970019765:+55 2197001-9765\n' //Nomor whatsapp kamu
            + 'END:VCARD'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					premium: '[❗] ESTE PEDIDO ÉS SOLO PARA *USUÁRIOS PREMIUMS*',
					mod: '[❗] ESTE PEDIDO ÉS ESPECÍFICO PARA USUARIO MOD DARK BOT*',
			...
			const ownerNumber = ["5521970019765@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5521970019765@s.whatsapp.net"]//ubah nomor lo
...
break
		   ...
		   hasil = `Ver los datos de gay ${gay}\n\n\nPercentage Gay : ${anu.persen}%\nAlerta!!! : gay, mas no ciego`
		   reply(hasil)
		   awat limitAdd(sender)
					break
                case 'nangis':
				...
				case 'meme':
					...
					break 
			    case 'kbbi':
					if (args.length < 1) return reply('Que es lo que quieres buscar?')
...
break
				...
				case 'testime':
					...
					break
		        case 'gay':		
	            	if (args.length < 1) return reply('etiquete sus amigos!')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					dark.sendMess(from, 'Que tan gay eres?: *'+rate+'*\n\nSu porcentage gay : '+ kl+'%', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'delete':
					...
					if (mentioned.length > 1) {
						teks = 'Comando recibido, excluir mensage :\n'
				...
				case 'openanime':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Activado con éxito el modo anime en este grupo ✔️')
				...
				case 'setfoto':
                    ...
                    reply('Se cambio con exito el icono del grupo')
                    break			
                case 'level':
                ...
                case 'leveling':
					...
					if (args[0] === 'on') {
                    if (isLevelingOn) return reply('*El comando de level yá estava activo*')
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('Antilink activado con exito✓')
						dark.sendMess(from,`*Atencion, antilink esta activo cualquier miembro que no sea admin y envie enlaces sera expulsado*`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('El anti-link fue desabilitado ')
						tag ini = anti.darkOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Desactivar antilink con éxito en este grupo ✔️')
					} else {
				...
				case 'setnomebot':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					name = body.slice(12)
					reply(`El nombre del bot fue activado con exito a  : ${name}`)
					break
					case 'clove':
					  if (args.length < 1) return reply('Cadê o texto, mano??')
                     if (args.length > 10) reurn reply('pelo menos 10 caracteres')
					 love = `${body.slice(7)}`
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Usted no es un miembro premium, entre en contacto con el proprietário o escriba *.Daftarvip* para adquirir el acceso premium!' ,text, { quoted: mek })
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok jefe, este chico: @${mentioned[0].split('@')[0]} Ahora es admin del grupo!`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ok, jefe este chico: @${mentioned[0].split('@')[0]} Ya no es admin!`, mentioned, true)
